...
- **课表打印接口(备选数据源)**:`POST /jsxsd/xskb/xskb_print.do?xnxq01id=<学期>&zc=` 返回 **Excel(.xls)** 文件。单元格文本 4 行结构:`课程名称 / 教师 / 周次(节次) / 教室`(多课以空行分隔),可用 JXL 库解析。当前 App 使用 HTML 源(已验证),此接口作为格式变化的备选
- **周次解析参考**:周次串格式 `"1-16周"` / `"6,8,10,12(周)"` / Excel 内 `"1-16[01-02节]"`;节次以 `[..]` 内数字为准
- 学籍卡片(可取姓名):`GET /jsxsd/grxx/xsxx`
- 会话失效表现:任何页面返回 853 字节错误页(含 `chucuole.gif` + `/jsxsd/xk/LoginToXk?method=exit`)→ 需重新登录
- **短时间高频登录会触发服务端持续限制**(数十分钟内所有请求返回错误页),App 正常使用(每天自动登录 1-2 次)无影响,但自动化测试需注意频率
- **WebVPN 相关**(调研结论):`*.webvpn.ncepu.edu.cn` 泛解析存在(域名改写:`-` 替换 `.`),但网关 202.204.70.3:443 从公网不可达,且门户未配置 Web 转发资源 → 纯客户端 WebVPN 通道不可行;校外使用请装官方 EasyConnect App(服务器 ycbg.ncepu.edu.cn)
